...
namespace FunctionApp1
{
    public class Function1
    {
        ...
        public async Task Run(
            [QueueTrigger("newqueue", Connection = "AzureWebJobsStorage")] QueueMessage message)
        {
            try
            {
                string? queueMessage = message.Body.ToString();
                _logger.LogInformation($"QueueTrigger triggered with message: {queueMessage}");

                string omdbApiKey = "a0e9070";
                ...
                {
                    _logger.LogWarning("Something happend problem in OMDb API");
                    return;
                }
